...
public class PatientCaseController {
	...
	public ResponseEntity<?> addPatientCase(@PathVariable String patientIdentifier ,@Valid @RequestBody PatientCase patientCase,BindingResult result){
		ResponseEntity<?> errorMap = mapValidationErrorService.mapValidationError(result);
		if (errorMap != null)
			return errorMap;
		PatientCase patientC = patientCaseService.addPatientCase(patientIdentifier,patientCase);
		return new ResponseEntity<PatientCase>(patientC, HttpStatus.OK);
		
		}
	
	@GetMapping("/getAllPatientCaseByIdentifier/{patientIdentifier}")
